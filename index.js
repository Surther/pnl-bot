...
async function calcForWallet(walletAddress, contractAddress, chain) {
    ...
    for (const hash of Object.keys(buyTxs)) {
        ...
        if (paid === 0) {
            try {
                ...
                if (tx && tx.value) {
                    paid = parseFloat(ethers.formatEther(tx.value));
                    source = "tx-value";
                }
            } catch (e) {
    ...
    for (const block of sellBlocks) {
        ...
        for (const [hash, blockNum] of Object.entries(sellTxs)) {
            ...
            // Nothing detected arriving. On a direct listing sale the buyer's
            // transaction carries the price, so fall back to that.
            if (got === 0) {
                try {
                    const tx = await provider.getTransaction(hash);
                    if (tx && tx.value) {
                        got = parseFloat(ethers.formatEther(tx.value));
                        source = "tx-value";
                    }
                } catch (e) {
                    console.log(`Sell tx ${hash} lookup failed:`, e.message);
                }
            }
