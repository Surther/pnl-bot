...
// Build a map of txHash -> total ETH moved, for a direction
async function buildEthMap(chain, direction, wallet) {
    const map = {};
    const categories = ["external", "internal"];
    for (const cat of categories) {
        try {
            ...
            const transfers = await getTransfers(chain, params);
            for (const t of transfers) {
                if (!t.hash || !t.value) continue;
                map[t.hash.toLowerCase()] = (map[t.hash.toLowerCase()] || 0) + Number(t.value);
            }
        } catch (e) {
            console.log(`ETH ${direction}/${cat} fetch skipped:`, e.message);
        }
    }
    return map;
}
...
async function calculatePnL(walletAddress, contractAddress, chain) {
    ...
    // NFT movements
    const nftIn = await getTransfers(chain, {
        toAddress: wallet,
    ...
    const nftOut = await getTransfers(chain, {
        fromAddress: wallet,
        contractAddresses: [contractAddress],
        category: ["erc721", "erc1155"],
        withMetadata: false,
    ...
    // ETH movements, keyed by tx hash (deduped per transaction)
    const [ethOutMap, ethInMap] = await Promise.all([
        buildEthMap(chain, "out", wallet),
        buildEthMap(chain, "in", wallet)
    ]);

    // Unique tx hashes so a multi-NFT tx is only counted once
    const buyHashes = [...new Set(nftIn.map(t => t.hash?.toLowerCase()).filter(Boolean))];
    const sellHashes = [...new Set(nftOut.map(t => t.hash?.toLowerCase()).filter(Boolean))];

    let investedEth = 0;
    for (const h of buyHashes) investedEth += ethOutMap[h] || 0;

    let realizedEth = 0;
    for (const h of sellHashes) realizedEth += ethInMap[h] || 0;

    // How many are still held
    const inCount = nftIn.length;
    const outCount = nftOut.length;
    const heldCount = Math.max(0, inCount - outCount);

    // Unrealized value of what's still held
    const floorEth = await getFloorPrice(contractAddress, chain);
    const positionEth = heldCount * floorEth;

    const pnlEth = realizedEth + positionEth - investedEth;
    const pnlUsd = pnlEth * ethPrice;
    const investedUsd = investedEth * ethPrice;
    const positionUsd = positionEth * ethPrice;
    const realizedUsd = realizedEth * ethPrice;

    const pnlPercent = investedEth > 0 ? (pnlEth / investedEth) * 100 : null;

    console.log(`PnL calc: invested=${investedEth}Ξ realized=${realizedEth}Ξ held=${heldCount} floor=${floorEth}Ξ`);

    return {
        pnlEth, pnlUsd, investedUsd, positionUsd, realizedUsd,
        pnlPercent, buyCount: inCount, sellCount: outCount,
        heldCount, floorEth, ethPrice, isProfit: pnlUsd >= 0
    };
}
