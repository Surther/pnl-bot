...
function loadFonts() {
    try {
        ...
        if (fs.existsSync(reg)) GlobalFonts.registerFromPath(reg, "PNL");
        if (fs.existsSync(bold)) GlobalFonts.registerFromPath(bold, "PNLBold");
        if (fs.existsSync(reg)) FONT = "PNL";
        console.log("Fonts loaded:", GlobalFonts.families.map(f => f.family).join(", "));
    } catch (e) {
...
async function generatePnLImage(data, collectionName, collectionImage, username) {
    ...
    ctx.font = `bold 26px PNLBold, ${FONT}, sans-serif`;
    ...
    ctx.font = `bold 72px PNLBold, ${FONT}, sans-serif`;
    ...
    ctx.fillStyle = "#ffffff";
    ctx.font = `bold 66px PNLBold, ${FONT}, sans-serif`;
